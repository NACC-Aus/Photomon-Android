...
class ExCamera1 extends CameraController implements Camera.PreviewCallback, Camera.ErrorCallback {
    ...
    ExCamera1(CameraView.CameraCallbacks callback) {
        super(callback);
    ...
    void capturePicture() {
        ...
        schedule(null, true, new Runnable() {
            ...
            public void run() {
                ...
                final int sensorToOutput = recomputeSensorToOutputOffset();
            ...
            public void run() {
                ...
                mCamera.setOneShotPreviewCallback(new Camera.PreviewCallback() {
                    ...
                    public void onPreviewFrame(final byte[] data, Camera camera) {
                        ...
                        mCamera.setPreviewCallbackWithBuffer(null); // Release anything left
                        mCamera.setPreviewCallbackWithBuffer(ExCamera1.this); // Add ourselves
                        mFrameManager.allocate(ImageFormat.getBitsPerPixel(mPreviewFormat), mPreviewSize);
                    }
                });
            }
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        Frame frame = mFrameManager.getFrame(data,
                ...
                recomputeSensorToOutputOffset(),
                mPreviewSize,
                mPreviewFormat);
        mCameraCallbacks.dispatchFrame(frame);
    }

    private int recomputeSensorToOutputOffset(){
        int outputOrientation;
        int displayOrientation = computeSensorToViewOffset();
        int mDeviceOrientation = 0;
        try {
            Field deviceOrientationField = getClass().getSuperclass().getDeclaredField("mDeviceOrientation");
            deviceOrientationField.setAccessible(true);
            mDeviceOrientation = deviceOrientationField.getInt(this);
        } catch (Throwable e) {
            e.printStackTrace();
        }
        if (mFacing == Facing.FRONT) {
            if(mDeviceOrientation == 0 || mDeviceOrientation == 180) {
                outputOrientation = (mSensorOffset - mDeviceOrientation + 360) % 360;
            }else {
                outputOrientation = Math.abs((mSensorOffset - mDeviceOrientation - displayOrientation) % 360);
            }
        } else {
            outputOrientation = displayOrientation;
        }

        LOG.i("outputOrientation = ", outputOrientation, "displayOrientation = ", displayOrientation, "mDeviceOrientation = ", mDeviceOrientation,
                "mSensorOffset = ", mSensorOffset);
    ...
    private void initMediaRecorder() {
        ...
        mMediaRecorder.setOrientationHint(recomputeSensorToOutputOffset());
